...
const SubjectCard: FC<ModalProps> = ({ subject }) => {
  const { image, subTitle, title, info, uid, difficult, type } = subject;
  ...
  return (
    <Styled.Container>
      <Styled.Image
        ...
        onClick={() =>
          navigate(`/quiz?category=${uid}&difficulty=${difficult}&type=${type}`)
        }
      />
      <Styled.AllInfoContainer>
        ...
        <Styled.More>
          ...
          <Styled.ActionButtons>
            <FaPlay
              onClick={() =>
                navigate(
                  `/quiz?category=${uid}&difficulty=${difficult}&type=${type}`
                )
              }
              size={15}
            />
            <FaPlus onClick={handleMyList} size={15} />
          </Styled.ActionButtons>
          <Styled.ExtraInfo>
            {info?.map((inf, index) => {
              return (
                <Styled.ExtraInfoInner>
                  {index === 0 ? "" : <GoDotFill size={10} />}
                  <Styled.ExtraInfoText>{inf}</Styled.ExtraInfoText>
                </Styled.ExtraInfoInner>
              );
            })}
          </Styled.ExtraInfo>
